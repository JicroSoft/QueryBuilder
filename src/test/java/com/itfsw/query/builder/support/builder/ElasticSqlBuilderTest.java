...
import com.itfsw.query.builder.ElasticsearchSqlQueryBuilderFactory;
import com.itfsw.query.builder.other.FileHelper;
import com.itfsw.query.builder.support.model.result.SqlQueryResult;

public class ElasticSqlBuilderTest {

	private static SqlBuilder builder;

    @BeforeClass
    public static void init() {
    	ElasticsearchSqlQueryBuilderFactory factory = new ElasticsearchSqlQueryBuilderFactory();
    ...
    public void testOperatorContains() throws IOException {
        ...
        Assert.assertEquals("username = match_query(?)", result.getQuery());
        Assert.assertEquals("Mistic", result.getParams().get(0));
        Assert.assertEquals("username = match_query('Mistic')", result.getQuery(true));
    }
